...
class Config {
  ...
  static load(environmentDir) {
    ...
    const defaultConfig = _safeLoad(path.join(environmentDir, 'default'));
    const envConfig = _safeLoad(path.join(environmentDir, env));
    const localConfig = _safeLoad(path.join(environmentDir, env + '.local'));
